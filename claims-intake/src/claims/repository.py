...
from claims.models import AcceptedNotification, RecordedNotification
...
class NotificationRepository:
    ...
    def __init__(self) -> None:
        self._saved: list[RecordedNotification] = []
        self._next_number = 1

    def record(self, notification: AcceptedNotification) -> RecordedNotification:
        ...
        and are never reissued. Only an AcceptedNotification can be written, so a
        rejected request cannot be stored (WI-0151 AC-3).
        """
        if not isinstance(notification, AcceptedNotification):
            raise TypeError("only an accepted notification can be recorded")
        year = datetime.now(UTC).year
        recorded = RecordedNotification(
            claim_reference=f"CLM-{year}-{self._next_number:06d}",
